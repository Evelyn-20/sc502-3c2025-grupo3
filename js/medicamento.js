...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const form = document.getElementById('formRegistrarMedicamento');
    if(form) {
        cargarFormas();
        cargarGrupos();
        cargarVias();
...
function listarMedicamentos() {
    fetch('MedicamentoController.php?action=list')
    .then(res => res.json())
    .then(data => {
        if(data.status === 'success') {
            const tbody = document.querySelectorAll('#tablaMedicamentos tbody');
            tbody.forEach(tb => tb.innerHTML = '');
            data.data.forEach(m => {
                const fila = `
                    <tr>
                        <td>${m.nombre}</td>
                        <td>${m.forma_farmaceutica}</td>
                        <td>${m.grupo_terapeutico}</td>
                        <td>${m.via_administracion}</td>
                        <td>${m.estado}</td>
                    </tr>
                `;
                tbody.forEach(tb => tb.innerHTML += fila);
            });
        }
    });
}
...
function cargarFormas() {
    fetch('MedicamentoController.php?action=getFormasFarmaceuticas')
    .then(res => res.json())
    .then(data => {
        const select = document.getElementById('id_forma_farmaceutica');
        select.innerHTML = '';
        data.data.forEach(f => {
            select.innerHTML += `<option value="${f.id_forma_farmaceutica}">${f.nombre}</option>`;
        });
    });
}

function cargarGrupos() {
    fetch('MedicamentoController.php?action=getGruposTerapeuticos')
    .then(res => res.json())
    .then(data => {
        const select = document.getElementById('id_grupo_terapeutico');
        select.innerHTML = '';
        data.data.forEach(g => {
            select.innerHTML += `<option value="${g.id_grupo_farmaceutico}">${g.nombre}</option>`;
        });
    });
}

function cargarVias() {
    fetch('MedicamentoController.php?action=getViasAdministracion')
    .then(res => res.json())
    .then(data => {
        const select = document.getElementById('id_via_administracion');
        select.innerHTML = '';
        data.data.forEach(v => {
            select.innerHTML += `<option value="${v.id_via_administracion}">${v.nombre}</option>`;
        });
    });
}

function registrarMedicamento() {
    const formData = new FormData(document.getElementById('formRegistrarMedicamento'));
    fetch('MedicamentoController.php?action=create', {
        method: 'POST',
        body: formData
    })
    .then(res => res.json())
