...
class Usuario {
    ...
    public function getAll($onlyActive = false) {
        $sql = "SELECT id_usuario, cedula_usuario, nombre, apellidos, correo, telefono, fecha_nacimiento, direccion, id_rol, id_estado FROM usuario";
        if ($onlyActive) { $sql .= " WHERE id_estado = 1"; }
    ...
    public function getById($id) {
        $stmt = $this->db->prepare("SELECT id_usuario, cedula_usuario, nombre, apellidos, correo, telefono, fecha_nacimiento, direccion, id_rol, id_estado FROM usuario WHERE id_usuario = ?");
    ...
    public function update($id, $data) {
        $fields = ["cedula_usuario","nombre","apellidos","correo","telefono","fecha_nacimiento","direccion","id_rol","id_estado"];
        $setParts = [];
        $values = [];
        $types = "";
        foreach ($fields as $f) {
        ...
        if (!empty($setParts)) {
            $sql = "UPDATE usuario SET ".implode(", ", $setParts)." WHERE id_usuario = ?";
