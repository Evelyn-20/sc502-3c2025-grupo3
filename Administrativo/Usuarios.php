...
async function cargarUsuarios() {
  const resp = await fetch('../router.php?action=listUsuarios');
  ...
  if(status==='success') {
    ...
    data.forEach(u => {
      ...
      tr.innerHTML = `
        ...
        <td>${u.estado}</td>
        <td>
          <button onclick="toggleUsuario(${u.id_usuario}, ${u.estado==1?0:1})">${u.estado==1?'Deshabilitar':'Habilitar'}</button>
        </td>
      `;
