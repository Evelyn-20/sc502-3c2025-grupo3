  ...
  <div class="container mt-5">
    <div class="row justify-content-center">
      <div class="col-md-10">
        ...
        <div class="row mb-4">
          <div class="col-4">
            <a type="button" class="btn btn-nuevo" href="RegistrarRol.html">+ Registrar Rol</a>
          </div>
        </div>
...
async function cargarRoles() {
  const resp = await fetch('../router.php?action=listRoles');
  ...
  if(status==='success') {
    ...
    data.forEach(r => {
      ...
      tr.innerHTML = `
        ...
        <td>${r.estado}</td>
        <td>
          <button onclick="toggleRol(${r.id_rol}, ${r.estado==1?0:1})">${r.estado==1?'Deshabilitar':'Habilitar'}</button>
        </td>
      `;
...
async function toggleRol(id, activar) {
  const fd = new FormData();
  fd.append('id_rol', id);
  const action = activar? 'enableRol':'disableRol';
  const resp = await fetch('../router.php?action='+action, {method:'POST', body:fd});
  const res = await resp.json();
  alert(res.message||res.status);
  cargarRoles();
}
window.addEventListener('DOMContentLoaded', cargarRoles);
</script>
